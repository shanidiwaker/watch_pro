...
export default class Slideshow extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      width: Dimensions.get('window').width,
      scrolling: false,
      isAutoPlay: this.props.isAutoLpay
  ...
  componentWillMount() {
    ...
    this.setState({isAutoPlay: this.props.isAutoLpay})
      ...
      <View style={[this.props.containerStyle, {height: height}]}>
        ...
        <ScrollView
          ...
          {this.props.dataSource.map((image, index) => {
            const isPlay = this.props.isAutoLpay
            const imageObject =
              typeof image.url === 'string' ? {uri: image.url} : image.url;
            const videoUrl = image.url.split('.').reverse()[0] === 'mp4' ? {uri: image.url} : ''
            ...
            const imageComponent = (
              videoUrl ? (
                <View key={index} style={{width: '100%', height: '100%'}}>
                  {/* <Text style={{color: '#fff'}}>{isPlay?.toString()}</Text> */}
                  {
            ...
            const imageComponentWithOverlay = (
              <View key={index} style={styles.containerImage}>
                <View style={styles.overlay}>
                  <Text style={{color: '#fff'}}>Hello123</Text>
                  <Image source={imageObject} style={{height, width}} />
                </View>
                {textComponent}
              </View>
