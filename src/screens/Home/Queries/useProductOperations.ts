...
async function fetchReels(
  ...
  try {
    ...
    if (id === 0) {
      ...
      url = `${config.BASE_URL}user/category_wise_reel`;
      response = await client.post(url, {
        category_id: id,
      });
    }
...
const useProductOperations = () => {
  ...
  // reels 
  const reelsbyCategory = async (id: number) => {
    try {
      const result = await fetchReels(id);
      queryClient.setQueryData(QueryKeys.homeProducts, result);
      return result;
    } catch (error) {
      Alert.alert('Error', 'Something went wrong'); // new
  ...
  return { likePeoduct, productsbyCategory, deleteProduct, reelsbyCategory };
};
